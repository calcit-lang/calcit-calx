...
pub unsafe fn run_buffer_adapter(
  ...
  match catch_unwind(AssertUnwindSafe(|| {
    ...
    Err(_) => {
      let _ = unsafe { write_output(output, b"calcit-calx buffer adapter panicked".to_vec()) };
...
mod tests {
  ...
  fn empty_buffer() -> CalcitFfiBuffer {
    CalcitFfiBuffer {
      ptr: ptr::null_mut(),
      len: 0,
      cap: 0,
  ...
  fn buffer_adapter_round_trips_and_rejects_invalid_boundaries() {
    let request = encode_edn(&Edn::List(vec![Edn::str("ok")])).expect("request");
    assert_eq!(
    ...
    let mut output = empty_buffer();
    assert_eq!(unsafe { run_buffer_adapter(ptr::null(), 1, &mut output, first_arg) }, 1);
    assert!(String::from_utf8(copy_buffer(output))
      .expect("UTF-8 error")
      .contains("pointer is null"));
    unsafe { calcit_ffi_buffer_free(output) };

    let scalar_request = encode_edn(&Edn::str("not-a-list")).expect("scalar request");
    let mut output = empty_buffer();
    assert_eq!(
      unsafe { run_buffer_adapter(scalar_request.as_ptr(), scalar_request.len(), &mut output, first_arg) },
      1
    );
    assert!(String::from_utf8(copy_buffer(output))
      .expect("UTF-8 error")
      .contains("must be a Cirru EDN list"));
    unsafe { calcit_ffi_buffer_free(output) };
  }
}
